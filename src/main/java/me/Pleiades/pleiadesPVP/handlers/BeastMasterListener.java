...
import me.Pleiades.pleiadesPVP.abilities.BeastMasterAbility;
...
public class BeastMasterListener implements Listener {
    ...
    public BeastMasterListener(PleiadesPVP pleiadesPVP){
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        ...
        if(!player.getScoreboardTags().contains("beast master")){
            return;
        }
        ...
        if(event.getAction() == Action.LEFT_CLICK_AIR || event.getAction() == Action.LEFT_CLICK_BLOCK){
            ...
            if(Objects.requireNonNull(player.getScoreboard().getObjective("Energy"))
                ...
                BeastMasterAbility.spawnHorse(player);
                System.out.println("Horse summoned");
            }
            ...
            if(Objects.requireNonNull(player.getScoreboard().getObjective("Energy"))
                ...
                Objects.requireNonNull(player.getScoreboard().getObjective("Energy"))
                        .getScore(ChatColor.YELLOW + "Energy: ").setScore(currentEnergy - 50);

                BeastMasterAbility.spawnDog(player);
                System.out.println("Dog summoned");
            }
    ...
    public void onEntityDeath(EntityDeathEvent event) {
        BeastMasterAbility.handleEntityDeath(event);
    }
